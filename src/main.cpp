...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    int currentBlock = pindexBest->nHeight;
    
    if(currentBlock > LAST_POW_BLOCK) {
        return 0;
    }
    
    if(currentBlock == 1) {
        nSubsidy = 1657545 * COIN;
    }
    else if(currentBlock < 150) {
        nSubsidy = 0 * COIN;
    }
    else if(currentBlock < 3601) {
        nSubsidy = currentBlock * 0.01 * COIN;
    }
    else {
        if(currentBlock > 7500) {
            if(currentBlock < 65000) {
                nSubsidy /= 2;
            }
            else if(currentBlock < 85000) {
                nSubsidy /= 3;
            }
            else if(currentBlock < LAST_POW_BLOCK) {
                nSubsidy /= 1.5;
            }
        }
        if((currentBlock%2) != 0) {
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
	...
	if((currentBlock != 0) && (currentBlock < POW_BLOCK_COUNT)) {
		nSubsidy = 10 * nCoinAge * nRewardCoinYear / 365 / COIN;
	}
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "UCoinTeam 29/01/15";
        CTransaction txNew;
        txNew.nTime = 1422486814;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1422486814;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5202914;
		if(fTestNet)
        {
            block.nNonce   = 5202914;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x25c3d29d9a2118c0d4e96d8a319bde22fe493e02ae86fffe52eb1e5add719551"));
        block.print();
